...
import { CoreModule, PlatformLanguageToken, PlatformWindowToken } from '@time/core';
import { throwIfAlreadyLoaded } from '@time/utils';
...
export class TimeCoreModule {
  constructor(
    @Optional()
    @SkipSelf()
    parentModule: TimeCoreModule
  ) {
    throwIfAlreadyLoaded(parentModule, 'TimeCoreModule');
  }
}
